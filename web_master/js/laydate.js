...
! function(a) {
    ...
    a.laydate = function(b) {
        ...
        }() + "</div>" + "</div>" + "</div>" + c.viewtb + '<div class="laydate_bottom">' + '<ul id="laydate_hms">' + '<li class="laydate_sj" id="laydate_sj">时间</li>' + "<li><input id='xiaoshi' >:</li>" + "<li><input id='fenzhong'>:</li>" + "<li><input id='miaoshu'></li>" + "</ul>" + '<div class="laydate_time" id="laydate_time"></div>' + '<div class="laydate_btn">' + '<a id="laydate_clear">清空</a>' + '<a id="laydate_today">今天</a>' + '<a id="laydate_ok">确认</a>' + "</div>" + (b.isv ? '<a href="http://sentsin.com/layui/laydate/" class="laydate_v" target="_blank">laydate-v' + laydate.v + "</a>" : "") + "</div>", d.body.appendChild(i), c.box = g("#" + h[0]), c.events(), i = null), c.follow(c.box), f.zIndex ? c.box.style.zIndex = f.zIndex : c.removeCssAttr(c.box, "z-index"), c.stopMosup("click", c.box), c.initDate(), c.iswrite(), c.check()
    }, c.reshow = function() {
	...
	var ls_tm = new Date();
	var ls_H = checktime(ls_tm.getHours());
	var ls_M = checktime(ls_tm.getMinutes());
	var ls_S = checktime(ls_tm.getSeconds());
	$('#xiaoshi').val(ls_H);
	$('#fenzhong').val(ls_M);
	$('#miaoshu').val(ls_S);
